...
FUNCTION mCrsTrkFunc, infile, outfile

  ;mCrsTrkFunc, 'C:\Users\Yen-Ben\Documents\IDL\CrsTrk\input.tif', 'C:\Users\Yen-Ben\Documents\IDL\CrsTrk\output.tif'

  ;this program is designed to correct the gradient seen in thermal images due to long flying time
  ;currently it's only working for one band
  ;it uses 1st degree polynomial fit (line) to fit the trend only in the X direction
...
;          RETURN
      ...
      ;readu, lun, inparr
      ;close, lun
      inparr = read_tiff(infile, GEOTIFF = GeoKeys)
      inparr = FLOAT(inparr)
    END
...
;          RETURN
  ...
  write_tiff, outfile, tifarr, GEOTIFF=GeoKeys, /SHORT
  RETURN, outfile
END